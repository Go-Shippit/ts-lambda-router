...
type HttpHandler<Url extends string, B, R extends Responses> = Handler<Url, B, R, void>;

export const Middleware = {
  of:
    <T>(impl: Handler<string, any, Responses, Handler<string, any, any, T>>) =>
    <Url extends string, B, R extends Responses>(handler: Handler<Url, B, R, T>): HttpHandler<Url, B, R> =>
      ((req, orig) => impl(req as any, orig, handler as any)) as HttpHandler<Url, B, R>,
};
...
const authedRoute = LambdaRouter.build(r =>
  r.get('/routes/{id}')(authMiddleware(async (req, original, t) => req.response(200, 'Hey')))
);
