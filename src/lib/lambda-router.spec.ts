...
describe('ApiHandler', () => {
  ...
  it('should handle the root route', async () => {
    const handler = LambdaRouter.build(r => r.get('/')((r, o) => r.response(200, 'OK')));
  ...
  it('should handle a root route with escaped characters', async () => {
    const handler = LambdaRouter.build(r => r.get('/{userId}')((r, o) => r.response(200, r.pathParams.userId)));
    const result = await testHandler(handler)({
      path: '/github%7C3257273',
      httpMethod: 'get',
      body: '',
    });
    expect(JSON.parse(result.body)).toBe('github|3257273');
    expect(result.statusCode).toBe(200);
  });

  it('should return 404 for unknown route', async () => {
    const handler = LambdaRouter.build(r =>
      r.get('/', { responsesSchema: { 200: Type.String() } })((r, o) => r.response(200, 'OK'))
    );
    const result = await testHandler(handler)({
      path: '/hello',
      httpMethod: 'get',
  ...
  it('should correct params for parameterised route', async () => {
    const handler = LambdaRouter.build(r =>
      r.get('/name/{name}/age/{age:int}')((r, o) => r.response(200, r.pathParams))
    );
    const result = await testHandler(handler)({
  ...
  it('should correct params for float parameterised route', async () => {
    const handler = LambdaRouter.build(r =>
      r.get('/name/{name}/age/{age:float}')((r, o) => r.response(200, r.pathParams))
    );
    const result = await testHandler(handler)({
      path: '/name/john/age/30',
      httpMethod: 'get',
  ...
  it('should correct 400 for parameterised post route with invalid body', async () => {
    const handler = LambdaRouter.build(r =>
      r
        .get('/name/{name}/age/{age:int}')((r, o) => r.response(200, r.pathParams))
        .post(
          '/name/{name}/age/{age:int}',
          Type.Object({
            creditCardNumber: Type.String(),
  ...
  it('should return 200 for for route with typed query strings and valid values', async () => {
    const handler = LambdaRouter.build(routes =>
      routes.get('/people?{ids:int[]}')((r, o) => r.response(200, { ...r.pathParams, ...r.queryParams }))
    );
    const result = await testHandler(handler)({
      path: '/people',
      httpMethod: 'get',
  ...
  describe('CORS Headers', () => {
    it("should add permissive cors headers when passed 'true' for cors config", async () => {
      const handler = LambdaRouter.build(routes => routes.get('/')((r, o) => r.response(200, 'OK')), {
        corsConfig: true,
      });
      const result = await testHandler(handler)({
        path: '/',
